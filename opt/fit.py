...
class DiesPerWafer(ABC): 
    def __init__(self) -> None:
        ...
        self.xspacing_default=.1
        self.yspacing_default=.1
    ...
    def fit(self, width=33, height=26, x_spacing=.1, y_spacing=.1, wafer_diameter=100, edge_exclusion_width=1, fit_type=3, search_depth=0): 
        ...
        def calcXYcen(offsets,ft):
            if ft==0:
                return (offsets[0]+Xoff*(width+x_spacing),
                        offsets[1]+Yoff*(height+y_spacing))
            elif ft==1:
                return (np.tile(offsets[1:], (2*Nx+1,1))+Xoff*(width+x_spacing),
                        offsets[0]+Yoff*(height+y_spacing))
            elif ft==2:
                return (offsets[0]+Xoff*(width+x_spacing),
                        np.tile(offsets[1:], (2*Ny+1,1)).transpose()+Yoff*(height+y_spacing))
        ...
        def trueoffsets(reducedoffset,ft):
            if ft==0:
                ...
                return np.concatenate((reducedoffset[:1],(reducedoffset[1:]>0.5)*(width+x_spacing)/2))
            elif ft==2:
                return np.concatenate((reducedoffset[:1],(reducedoffset[1:]>0.5)*(height+y_spacing)/2))
        ...
        start = time.time()
        if fit_type==0:
            fit=optimize.dual_annealing(withpartialscore, [(0,(width+x_spacing)/2), (0, (height+y_spacing)/2)],
            ...
            fit= optimize.dual_annealing(fullonly_rowshift,
                                                    ...
                                                    maxiter=maxiter_shift,no_local_search=self.no_local_search_shift)
            ...
            fit0=optimize.dual_annealing(withpartialscore, [(0,(width+x_spacing)/2), (0, (height+y_spacing)/2)],
        ...
        print('\nCentering output:')
        if fittype==0:
            fit2 = optimize.minimize(Rmax,fit_offsets,bounds=[(0,(width+x_spacing)/2), (0, (height+y_spacing)/2)],
                                        args=(fittype,fit_valid))
            centered_offsets=fit2.x
        elif fittype==1:
            fit2 = optimize.minimize_scalar(Rmax,bounds=(0, (height+y_spacing)/2),args=(fittype,fit_valid))
            centered_offsets=fit_offsets
            centered_offsets[0]=fit2.x
        elif fittype==2:
            fit2 = optimize.minimize_scalar(Rmax,bounds=(0, (width+x_spacing)/2),args=(fittype,fit_valid))
            centered_offsets=fit_offsets
            centered_offsets[0]=fit2.x
        print(fit2)
        ...
        return area_utilization, min_diameter, start, end, partial, Nfit, Xcen, Ycen, valid
    ...
    def run_ui(self):
        ...
        edgeexclusionwidth_s=tk.StringVar(value=str(self.edge_exclusion_width_default))
        ...
        def execute():
            ...
            area_utilization, min_diameter, start, end, partial, Nfit, Xcen, Ycen, valid = self.fit(
                width=width, 
                height=height, 
                x_spacing=xspacing, 
                y_spacing=yspacing,
            ...
            ax.add_patch(Circle((0,0),wafer_diameter/2-edge_exclusion_width,edgecolor = 'k',ls='--',fill=0))
