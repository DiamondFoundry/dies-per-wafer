...
def run_ui():
    ...
    check_grid = tk.Checkbutton(
        frame1, text="Uniform Grid", variable=boolvar_grid, onvalue=True, offvalue=False
    )
    ...
    check_ShiftRows = tk.Checkbutton(
        frame1,
        text="Shift Rows",
        variable=boolvar_ShiftRows,
        onvalue=True,
        offvalue=False,
    )
    ...
    check_ShiftCols = tk.Checkbutton(
        frame1,
        text="Shift Columns",
        variable=boolvar_ShiftCols,
        onvalue=True,
        offvalue=False,
    )
    ...
    check_ShiftRot = tk.Checkbutton(
        frame1,
        text="Shift & Rotate Rows",
        variable=boolvar_ShiftRot,
        onvalue=True,
        offvalue=False,
    )
    ...
    check_symmetry = tk.Checkbutton(
        frame1,
        text="X & Y Mirror Symmetry",
        variable=boolvar_symmetry,
        onvalue=True,
        offvalue=False,
    )
    ...
    def execute():
        ...
        extraoutput.set(
            "area utilization = {:.2%}\nminimum wafer diameter = {:.1f}\nsearch time: {:.2f} sec".format(
                area_utilization,
                math.ceil(10 * dpw.final_diameter) / 10,
                dpw.end - dpw.start,
            )
        )
        ...
        partial = np.logical_and(
            np.any(dpw.V2**0.5 <= dpw.ewr, axis=0), np.logical_not(valid)
        )

        ax.clear()
        title_text = "die number = {:n}   Fit Type = {}{}\nwidth = {:n}   height = {:n}\nx spacing = {:n}   y spacing = {:n}\nwafer diameter = {:n}   edge exclusion = {:n}".format(
            dpw.Nfit,
            ["Uniform Grid", "Shift Rows", "Shift Columns", "Shifted & Rotated"][
                dpw.fittype
            ],
            ["", " + Symmetric"][symmetric],
            width,
        ...
        for idx, _ in np.ndenumerate(dpw.Xcorner):
            if partial[idx]:
                ax.add_patch(
                    Rectangle(
                        (dpw.Xcorner[idx], dpw.Ycorner[idx]),
                        dpw.Xextent[idx],
                        dpw.Yextent[idx],
